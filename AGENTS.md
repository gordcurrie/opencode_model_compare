...
- **Models sometimes write to subdirectories** (e.g. `./output/`) instead of the working directory root. The benchmark now handles this: `generateCode` uses `filepath.WalkDir` recursively and returns the shallowest `.go` file's directory as the effective compile root. Prior to this fix (added Feb 22 2026), any file written to a subdirectory was silently missed, causing a false negative.
- **Stale `go.mod` from a prior run** can survive `make clean` if the model's working directory wasn't fully removed. `initGoModule` now checks for an existing `go.mod` and skips `go mod init` if one is present. Prior to this fix, a stale `go.mod` caused an immediate compile failure false negative.
